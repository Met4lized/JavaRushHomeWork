...
public class Solution {
    ...
    public static void main(String[] args) {
        try (BufferedReader fileName = new BufferedReader(new InputStreamReader(System.in));
            ...
            while (reader.ready()) {
                dataList.addAll(Arrays.asList((reader.readLine().split(" "))));
            }
            for (int i = 0; i < dataList.size(); i++) {
                String first = dataList.get(i);
                String second = new StringBuffer(first).reverse().toString();
                if (dataList.indexOf(second) != -1 & !first.equals("") & !first.equals(second)) {
                    result.add(new Pair(first, second));
                    dataList.remove(first);
                    dataList.remove(second);
                }
            }
